      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Source Document</label>
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Questions</label>
              <input type="number" defaultValue={10} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" />
            </div>
          </div>
          ...
          <Card key={q.id}>
            <CardContent className="p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs text-gray-500">Confidence: {Math.round(q.confidence * 100)}%</span>
              </div>
              <p className="font-medium text-gray-900 mb-3">{q.text}</p>
              <div className="grid grid-cols-2 gap-2 mb-3">
                {q.options.map((opt, i) => (
                  <div key={i} className={`p-2 rounded text-sm ${i === q.correct ? "bg-success/10 border border-success/20" : "bg-gray-50"}`}>
                    {String.fromCharCode(65 + i)}. {opt} {i === q.correct && "✓"}
                  </div>
                ))}
              </div>
              <p className="text-xs text-gray-500 mb-2">Source: {q.source}</p>
              <p className="text-sm text-gray-600 italic mb-3">{q.explanation}</p>
